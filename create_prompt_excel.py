...
apply_cell(ws1, 1, 1, "プロンプト生成ツール ／ 入力シート",
           font=Font(bold=True, color="FFFFFF", size=14),
           fill=HEADER_FILL, align=CENTER)

# 説明行
ws1.merge_cells("A2:C2")
apply_cell(ws1, 2, 1, "以下の3項目を入力（または選択）してください。シート2にプロンプトが自動生成されます。",
           font=Font(size=10, color="555555", italic=True),
           align=LEFT)

# ヘッダー行
for col, text in enumerate(["No.", "項目名", "入力値"], start=1):
    apply_cell(ws1, 3, col, text, font=HEADER_FONT, fill=HEADER_FILL,
               align=CENTER, border=BORDER)

# ─ 行1: 案件名 ─
apply_cell(ws1, 4, 1, "1", font=LABEL_FONT, fill=LABEL_FILL, align=CENTER, border=BORDER)
apply_cell(ws1, 4, 2, "案件名", font=LABEL_FONT, fill=LABEL_FILL, align=LEFT, border=BORDER)
c_anken = ws1.cell(row=4, column=3, value="（例）育児休業申請対応")
c_anken.font   = INPUT_FONT
c_anken.fill   = INPUT_FILL
c_anken.alignment = LEFT
c_anken.border = BORDER

# ─ 行2: 担当者 ─
apply_cell(ws1, 5, 1, "2", font=LABEL_FONT, fill=LABEL_FILL, align=CENTER, border=BORDER)
apply_cell(ws1, 5, 2, "担当者（総務/人事/経理）\nおよび決裁者（社長/役員）",
           font=LABEL_FONT, fill=LABEL_FILL, align=LEFT, border=BORDER)
c_tanto = ws1.cell(row=5, column=3, value="総務担当者および社長")
c_tanto.font   = INPUT_FONT
c_tanto.fill   = INPUT_FILL
c_tanto.alignment = LEFT
c_tanto.border = BORDER

# ─ 行3: 成果物タイプ（ドロップダウン） ─
apply_cell(ws1, 6, 1, "3", font=LABEL_FONT, fill=LABEL_FILL, align=CENTER, border=BORDER)
apply_cell(ws1, 6, 2, "成果物タイプ\n（ドロップダウンから選択）",
           font=LABEL_FONT, fill=LABEL_FILL, align=LEFT, border=BORDER)
c_type = ws1.cell(row=6, column=3, value="回答資料")
c_type.font   = INPUT_FONT
c_type.fill   = INPUT_FILL
c_type.alignment = LEFT
...
PROMPT_FILL  = PatternFill("solid", fgColor="F2F9FF")
PROMPT_FONT  = Font(size=10, name="Meiryo")

ws2.merge_cells("A1:B1")
apply_cell(ws2, 1, 1, "生成プロンプト（コピーしてご利用ください）",
           font=Font(bold=True, color="FFFFFF", size=13),
           fill=HEADER_FILL, align=CENTER)

# プロンプトを数式で組み立てる
# シート1の入力セル: C4=案件名, C5=担当者, C6=成果物タイプ
SHEET1 = "シート1_入力"

# CHAR(10) = 改行。セルの wrap_text を True にする必要がある
formula = (
    '="# 前提条件"&CHAR(10)'
    '&"- タイトル: "&\'シート1_入力\'!C4&"顧問先問い合わせ対応"&CHAR(10)'
    ...
    '&"- 読み手: 顧問先の"&\'シート1_入力\'!C5&"。忙しく、結論と次のアクションが欲しい。"&CHAR(10)'
    '&"- 成果物タイプ: "&\'シート1_入力\'!C6&CHAR(10)'
    '&CHAR(10)'
    '&"# 実行指示"&CHAR(10)'
    '&"[#変数設定]・[#参考情報]に基づき、"&\'シート1_入力\'!C6&" を作成してください。"&CHAR(10)'
    '&"成果物タイプは "&\'シート1_入力\'!C6&" に従い、該当する[#出力フォーマット]を厳守してください。"&CHAR(10)'
    '&CHAR(10)'
    ...
    '&CHAR(10)'
    '&"# 出力フォーマット"&CHAR(10)'
    '&\'シート1_入力\'!C6&" に応じて、以下のいずれか「1つだけ」を採用し、他形式は混ぜない。"'
)

ws2.merge_cells("A2:B2")
c_prompt = ws2["A2"]
c_prompt.value = formula
c_prompt.font = PROMPT_FONT
c_prompt.fill = PROMPT_FILL
c_prompt.alignment = Alignment(
    horizontal="left", vertical="top", wrap_text=True
)
c_prompt.border = Border(
...
ws2.row_dimensions[2].height = 600   # 縦に十分な高さ
